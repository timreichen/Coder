...
export class WeakSet16DataType extends DataType {
  validate(data) {
    return isType(data, WeakSet) && data.size <= UINT_16_MAX_VALUE
  }
...
export class WeakSet32DataType extends DataType {
  validate(data) {
    return isType(data, WeakSet) && data.size <= UINT_32_MAX_VALUE
  }
  encode(encoder: Encoder, data) {
    const length = data.size
    const idBuffer = Encoder.uInt8ToBuffer(this.id)
